...
sudo dnf install akmod-nvidia xorg-x11-drv-nvidia-cuda @base-x xorg-x11-server-Xorg xorg-x11-drv-{evdev,intel,synaptics,libinput} xorg-x11-xinit
gvfs-fuse gvfs-backends make automake gcc gcc-c++ kernel-devel libX11-devel libXft-devel libXinerama-devel tlp tlp-rdw thermald git cups pulseaudio alsa-plugins-pulseaudio pulseaudio-module-x11 pulseaudio-utils kernel-modules-extra libXrandr libXrandr-deve htop thunar pavucontrol xbacklight powertop udiskie ffmpeg nano ncmpcpp libmpd volumeicon dunst w3m w3m-img feh lxappearance firefox conky mpv unzip gimp rsync leafpad steam compton transmission darkplaces quake3 xclip evince asunder easytag simple-scan NetworkManager network-manager-applet nm-connection-editor NetworkManager-wifi
VirtualBox virtualbox-guest-additions file-roller && \
